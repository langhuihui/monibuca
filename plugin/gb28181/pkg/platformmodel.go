...
// BeforeCreate GORM钩子，在创建记录前确保DeletedAt为nil
func (p *PlatformModel) BeforeCreate(tx *gorm.DB) error {
...
type PlatformModel struct {
	ID                      uint               `gorm:"primaryKey;autoIncrement;column:id" json:"id"`                                                         // ID表示主键ID
	Enable                  bool               `gorm:"column:enable" json:"enable"`                                                                          // Enable表示该平台配置是否启用
	Name                    string             `gorm:"column:name;omitempty" json:"name"`                                                                    // Name表示平台的名称
	ServerGBID              string             `gorm:"column:server_gb_id;type:varchar(32);uniqueIndex:idx_platform_gb_deleted;omitempty" json:"serverGBId"` // ServerGBID表示SIP服务器的国标编码
	...
	DeviceGBDomain          string             `gorm:"column:device_gb_domain;omitempty" json:"deviceGBDomain"`                                              // DeviceGBDomain表示设备的国标域
	...
	RegisterInterval        int                `gorm:"column:register_interval;omitempty" json:"registerInterval"`                                           // RegisterInterval表示注册间隔，单位为秒
	KeepTimeout             int                `gorm:"column:keep_timeout;omitempty" json:"keepTimeout"`                                                     // KeepTimeout表示心跳超时时间，单位为秒
	MaxTimeoutCount         int                `gorm:"column:max_timeout_count;omitempty" json:"maxTimeoutCount"`                                            // MaxTimeoutCount表示最大心跳超时次数
	...
	DeletedAt               gorm.DeletedAt     `yaml:"-" gorm:"uniqueIndex:idx_platform_gb_deleted"` // 联合唯一索引：软删除后允许相同ServerGBID但DeletedAt不同的记录
