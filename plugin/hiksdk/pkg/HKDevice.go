...
func RealDataCallBack_V30(lRealHandle C.LONG, dwDataType C.DWORD, pBuffer *C.BYTE, dwBufSize C.DWORD, pUser unsafe.Pointer) {
	...
	// 	// 将pUser转换为HKDevice指针并调用dmp方法
	// 	if pUser != nil {
	// 		device := (*HKDevice)(pUser)
	// 		// 使用unsafe.Pointer高效转换[]C.BYTE为[]byte
	// 		goBuffer := (*[1 << 30]byte)(unsafe.Pointer(&buffer[0]))[:len(buffer):len(buffer)]
	// 		device.dmp(goBuffer)
	// 	}
	...
	// }
	// size := int(dwBufSize)
	// fmt.Println("lRealHandle", lRealHandle)
	// fmt.Println("dwDataType", dwDataType)
	// fmt.Println("pBuffer", pBuffer)
	// 使用slice而不是array来处理变长数据
	// if size > 0 && pBuffer != nil {
	// 	// 将C指针转换为Go的byte切片
	// 	buffer := (*[1 << 30]C.BYTE)(unsafe.Pointer(pBuffer))[:size:size]
	// 	fmt.Println("Buffer length:", len(buffer))
	...
	// 	// 可以根据需要处理buffer中的数据
	// }
	// fmt.Println("dwBufSize", dwBufSize)
}
