...
func (p *MP4Plugin) List(ctx context.Context, req *pb.ReqRecordList) (resp *pb.ResponseList, err error) {
	...
	for _, stream := range streams {
		resp.List = append(resp.List, &pb.RecordFile{
			Id:         uint32(stream.ID),
...
func (p *MP4Plugin) download(w http.ResponseWriter, r *http.Request) {
	...
	query := `((start_time BETWEEN ? AND ?)
							...
							OR (? BETWEEN start_time AND end_time)) AND stream_path=? AND record_mode=0`
	p.DB.Where(query, startTime, endTime, startTime, endTime, startTime, endTime, streamPath).Find(&streams)
